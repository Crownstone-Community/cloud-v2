...
export class EnergyDataProcessor {
  ...
  async processMeasurements(sphereId: string) {
    ...
    while (iterationRequired) {
      let energyData = await Dbs.stoneEnergy.find({
        where: {processed: false, sphereId: sphereId},
        limit: iterationSize,
        order: ['timestamp ASC']
      });
  ...
  async _processStoneEnergy(stoneId: string, energyData: EnergyData[]) {
    ...
    if (samples.length > 0) {
      ...
      await Dbs.stoneEnergy.deleteAll({stoneId: stoneId, processed:true, timestamp: {lt: lastDatapoint.timestamp}});
    }
  }
  ...
  async _processAggregations(
    ...
    while (iterationRequired) {
      samples = [];
      let processedPoints = await Dbs.stoneEnergyProcessed.find({where: { stoneId: stoneId, interval: intervalData.basedOnInterval, timestamp: {gt: fromDate}}, limit: iterationSize, order: ['timestamp ASC'] });
      ...
      for (let i = 0; i < processedPoints.length; i++) {
        ...
        if (isONsamplePoint) {
          ...
          if (previousSamplePointFromCurrent == previousSamplePointFromLast) {
            ...
            let dJms = dJ / dt;
            let elapsedSamplePoints = intervalData.getNumberOfSamplePointsBetween(previousSamplePointFromCurrent, previousSamplePointFromLast);
...
async function processDataPairSingleNew(
  ...
  async function wrapUp() {
    ...
    await Dbs.stoneEnergy.update(nextDatapoint).catch((e) => {log.error("Error persisting processed boolean on datapoint", e);})
  }
